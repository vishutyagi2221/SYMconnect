...
app = FastAPI(title="SYMconnect", version="0.1.0")
...
async def viewer_socket(websocket: WebSocket) -> None:
    ...
    try:
        ...
        session = await sessions.get(session_id)
        if session is None:
            await close_with_error(websocket, "Session not found.")
            return
        if session.pairing_hash != hash_pairing_code(pairing_code):
            await close_with_error(websocket, "Pairing code rejected.")
            return
